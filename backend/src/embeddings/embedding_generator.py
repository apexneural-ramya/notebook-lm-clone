            ...
            try:
                ...
                try:
                    ...
                    if SENTENCE_TRANSFORMERS_AVAILABLE:
                        ...
                        local_model_path = str(backend_dir / ".embedding_model")
                        
                        # Check for locally downloaded model in backend directory (workaround for Windows)
                        if os.path.exists(local_model_path) and os.path.exists(os.path.join(local_model_path, "config_sentence_transformers.json")):
                            logger.info(f"Using local model from backend directory: {local_model_path}")
                            self.model = SentenceTransformer(local_model_path)
                        else:
                            # Try downloading to backend directory first
                            try:
                                from huggingface_hub import snapshot_download
                                logger.info("Downloading model to backend directory...")
                                # local_model_path already set above
                                snapshot_download(
                                    repo_id=model_name,
