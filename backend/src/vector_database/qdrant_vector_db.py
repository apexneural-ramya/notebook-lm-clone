...
class QdrantVectorDB:
    def __init__(
        ...
        # Default to backend/qdrant_db if not specified
        if db_path is None:
            # Get backend directory (parent of src)
            backend_dir = Path(__file__).parent.parent.parent
            db_path = str(backend_dir / "qdrant_db")
    ...
    def insert_embeddings(self, embedded_chunks: List[EmbeddedChunk], user_id: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            for embedded_chunk in embedded_chunks:
                ...
                # Add user_id if provided for user-specific isolation
                if user_id:
    ...
    def search(
        ...
        try:
            ...
            for point in points:
                ...
                # Skip if user_id doesn't match (additional safety check)
    ...
    def delete_source(self, source_file: str, user_id: Optional[str] = None) -> int:
        """
        Delete all points for a specific source file, optionally filtered by user_id.
        ...
        try:
            ...
            # First, get all point IDs that match the filter
            ...
            if not points:
                logger.info(f"No points found for source: {source_file}")
                return 0
            ...
            logger.info(f"Deleted {deleted_count} points for source: {source_file}")
            return deleted_count
            
        except Exception as e:
            logger.error(f"Error deleting source {source_file}: {str(e)}", exc_info=True)
            raise
...
if __name__ == "__main__":
    ...
    try:
        # Use backend/data path
        backend_dir = Path(__file__).parent.parent.parent
        chunks = doc_processor.process_document(str(backend_dir / "data" / "raft.pdf"))
